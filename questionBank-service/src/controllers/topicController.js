 ...
 *
 * Returns all topics that have at least one question, including their
 * available difficulty levels (Easy, Medium, Hard) in sorted order.
 */
exports.getAllTopicsWithDifficulty = async (req, res) => {
  try {
    const result = await db.query(
      `SELECT 
        t."topicId", 
        t."topicName", 
        ARRAY_AGG(DISTINCT q."difficulty" ORDER BY q."difficulty") AS "difficulties"
       FROM "topic" t
       INNER JOIN "question_bank" q ON t."topicId" = q."topicId"
...
exports.deleteTopic = async (req, res) => {
  ...
  try {
    ...
    if (questions.rows[0].count > 0) {
      return res.status(400).json({
        error:
          "Error deleting topic: Please delete questions associated with this topic first",
      });
    }

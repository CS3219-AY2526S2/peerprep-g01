...
const DIFFICULTIES = [
  { label: "Easy", emoji: "😊", Subtitle: "Perfect for beginners", colors: "#00d607", bg: "#b3efb5" },
  { label: "Medium", emoji: "😐",  Subtitle: "Challenge yourself", colors: "#dd8603", bg: "#f9c87d"},
  { label: "Hard", emoji: "🔥", Subtitle: "For Experts", colors: "#fe1100", bg: "#fc887f" },
]
...
function UserHomePage() {
  ...
  return (
    <Box sx={{ minHeight: "100vh", bgcolor: "#f5f5f5", pb: 6 }}>
      ...
      <Box sx={{maxWidth: 1000, mx: "auto", px: 2 }}>
        <Card variant="outlined" sx={{ borderRadius: 3, p: 1}}>
          <CardContent>
            ...
            {topicLoading ? (
              <Typography variant="caption" color="text.secondary"> Loading Topics...</Typography>
            ) : (
                ...
                {topics.map((t) => {
                  ...
                  return(
                    <Grid size={3} key={t.topicId}>
                      <Box onClick={() => setSelectedTopic(t.topicName)} sx={{
                        border: selected ? "2px solid #1976d2" : "1px solid #e0e0e0",
                        bgcolor: selected ? "#97d2fb" : "#fff",
                        borderRadius: 2, p: 1.5, textAlign: "center", cursor: "pointer",
                        transition: "all 0.15s", "&:hover": {borderColor: "#1972d2"},
                      }}>
            ...
            {DIFFICULTIES.map((d) => {
              ...
              return(
                <Grid key={d.label} size={4}>
                  <Box onClick = {() => setSelectedDifficulty(d.label)} sx={{
                    border: selected ? `2px solid ${d.colors}`: "1px solid #e0e0e0",
                    ...
                    <Typography variant="caption" color="text.secondary">{d.Subtitle}</Typography>
                  </Box>
                </Grid>
              )
            }
          ...
          <Box sx={{display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 1, mb: 2}}>
            <Typography variant="body2" fontWeight={600} fontSize={13}>Selected: </Typography>
            {selectedTopic && (
              <Chip label={selectedTopic} size="small" color="primary" variant="outlined"/>
            )}
            <Chip label={selectedDifficulty} size="small" color="primary" variant="outlined"/>
          </Box>

          {/*Button*/}
          <Box sx={{ textAlign: "center" }}>
            <Button variant="contained" size="large" startIcon={<GroupsIcon />}
              sx={{ borderRadius: 3, px: 4, textTransform: "none", fontWeight: 600, py: 1.5 }}>
      ...
      {/* How it works */}
      <Box sx={{ mx: "auto", mt: 3, maxWidth: 940, bgcolor: "#c6dcf8", borderRadius: 3, p: 4, textAlign: "center" }}>
        <Typography variant="h6" fontWeight={700} sx={{ mb: 3 }}>How It Works</Typography>
          ...
          {HOW_IT_WORKS.map((item) => (
            <Grid size={3} key={item.step}>
              <Box sx={{
                ...
                fontWeight: 700, fontSize:18, mx: "auto", mb: 1,
              }}>
                {item.step}
              </Box>
